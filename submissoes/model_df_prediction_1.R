...
rf <- makeLearner("classif.randomForest", predict.type = "response", par.vals = list(mtry = 3, ntree = 200, importance = TRUE,
                                                                                     cutoff = c(0.6, 0.4)))
...
# gravando a saida
# write.csv(df_predictions,file="submissoes/df_predictions_2.csv", row.names=FALSE)
